...
const getChunkDir = (fileHash) => path.resolve(UPLOAD_DIR, `chunkCache_${fileHash}`)
...
app.post('/upload', async (req, res) => {
    ...
    try {
        ...
        // 这个库的主要作用就是去将数据添加进form-data和解析上传的form-data
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            console.log(`文件hash：${fileHash}，切片hash：${chunkHash}，文件名：${fileName}`)

            // files是传过来的文件所在的真实路径以及内容
            const { chunkFile } = files
            console.log('上传服务器后切片文件的临时地址：', chunkFile[0].path)

            // 创建一个临时文件目录用于 临时存储所有文件切片
            const chunkCache = getChunkDir(fileHash)
            console.log('临时目录：', chunkCache)
            ...
            await fse.move(chunkFile[0].path, path.resolve(`${chunkCache}`, `${chunkHash}`), {
                overwrite: true,
            })
            res.send({
                code: 0,
...
const mergeFileChunk = async (chunkSize, fileHash, filePath) => {
    try {
        ...
        for (let index = 0; index < chunkPaths.length; index++) {
            ...
            let writeStream = fse.createWriteStream(filePath, { start: index * chunkSize })
...
const pipeStream = (path, writeStream) => {
    return new Promise((resolve) => {
        ...
        // 在一个指定位置写入文件流 [切片文件1] → 读取流 → 管道 → 写入流 → [最终文件位置1]
