...
- 追剧模式（详情页播放组件）：是和首页几乎一样的，除了播放的视频列表全部转换为剧集内的，并且不会走鉴权接口，而是会直接请求地址。减少请求的发出。

- 详情页播放组件属于特殊页，与详情页一样，会单开一个路由

- 懒加载路由

- 优化keep-alive组件包裹的组件，不用依靠组件的生命周期来加载数据

- Home的监听键盘滚轮的事件也需要修改为不依组件生命周期的形式

- 优化加载placeholder的出现时机，需要与video.js结合

- 将所有的剧集信息采集下来，保存在本地，不去请求了

- 优化详情页的样式（有点丑，不够高级

- 添加动态背景水印

- cookie，头像上传，npm包，发布开源视频

- 优化并发请求的性能问题

- 添加微信分享功能，这个功能就抽离朱来，做为一个单独的js模块来实现，不在组件内部写
